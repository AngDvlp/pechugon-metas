...
export default function GerenteUsuarios() {
  ...
  const [supSucursales, setSupSucursales] = useState({}) // supervisor_id -> [sucursal_id]
  ...
  async function handleAdd(e) {
    ...
    // 1. Crear usuario en Auth de Supabase (admin)
    // NOTA: En producción esto debería hacerse desde un edge function con service_role key
    // Por simplicidad, usamos signUp y luego insertamos en usuarios
    ...
    if (authErr) {
      setMsg({ tipo: 'error', texto: 'Error al crear cuenta: ' + authErr.message })
      setSaving(false)
      return
    }
    ...
    if (!uid) {
      setMsg({ tipo: 'error', texto: 'No se pudo obtener el ID del usuario' })
      setSaving(false)
      return
    }
    ...
    if (dbErr) {
      setMsg({ tipo: 'error', texto: 'Error al guardar usuario: ' + dbErr.message })
      setSaving(false)
      return
    }
  ...
  async function handleAsignarSucursal(supervisorId, sucursalId) {
    ...
    if (ya) return
    const { error } = await supabase.from('supervisor_sucursales').insert({ supervisor_id: supervisorId, sucursal_id: sucursalId })
    if (!error) await load()
  }

  async function handleQuitarSucursal(supervisorId, sucursalId) {
    await supabase.from('supervisor_sucursales')
      .delete()
      ...
      {showForm && (
        <form className={styles.formCard} onSubmit={handleAdd} noValidate>
          ...
          <div className={styles.field}>
            ...
            <input className={styles.input} type="email" placeholder="correo@pechugon.com"
            ...
            return (
              <div key={u.id} className={styles.userCard}>
                ...
                {rolNombre === 'encargado' && u.sucursales && (
                  <p className={styles.sucAsignada}>Sucursal: {u.sucursales.nombre}</p>
                )}
                    ...
                    <div className={styles.supTags}>
                      {misSupSucs.map(ss => (
                        <span key={ss.sucursal_id} className={styles.supTag}>
                          ...
                          <button className={styles.quitarBtn} onClick={() => handleQuitarSucursal(u.id, ss.sucursal_id)}>×</button>
                        </span>
                      ))}
                    </div>
                    {misSupSucs.length < 5 && (
                      <select
                        ...
                        defaultValue=""
                        onChange={e => {
                          handleAsignarSucursal(u.id, e.target.value)
                          e.target.value = ''
                        }}
                      >
                        <option value="">+ Asignar sucursal…</option>
                        {sucursales
                          .filter(s => !misSupSucs.find(ss => ss.sucursal_id === s.id))
